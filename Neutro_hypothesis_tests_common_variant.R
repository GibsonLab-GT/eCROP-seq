# =============================================================================
# SCEPTRE + T-TEST — eCROPseq Pool for Neutro pools
# =============================================================================
...
EXPR_CSF3R  <- as.numeric(GetAssayData(data, assay = "RNA", layer = "data")["CSF3R",  ])
EXPR_S100A8 <- as.numeric(GetAssayData(data, assay = "RNA", layer = "data")["S100A8", ])

is_neutro <- EXPR_CSF3R  >= mean(EXPR_CSF3R) |
  EXPR_S100A8 >= mean(EXPR_S100A8)

data <- data[, is_neutro]
message(sprintf("Neutrophil cells retained: %d (%.1f%%)",
                ncol(data), 100 * mean(is_neutro)))
