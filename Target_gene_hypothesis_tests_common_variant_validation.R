...
#read in and filter file for target gene of interest with corresponding gRNA information
...
# and separate in group without one of those gRNA
data_sub_no_gRNA_for_target <- subset(x = data, idents = "TRUE")

# or with one of them
data_sub_gRNA_for_target <- subset(x = data, idents = "FALSE")
...
EXPR = GetAssayData(object=data_sub_neg, assay="RNA",slot="data")[RSID,]
