...
# =============================================================================
# 7b. MACROPHAGE FILTER
# =============================================================================

EXPR_CD14 <- as.numeric(GetAssayData(data, assay = "RNA", layer = "data")["CD14", ])
EXPR_CD68 <- as.numeric(GetAssayData(data, assay = "RNA", layer = "data")["CD68", ])

is_macro  <- EXPR_CD14 >= mean(EXPR_CD14) | EXPR_CD68 >= mean(EXPR_CD68)
data      <- data[, is_macro]
message(sprintf("Macrophage cells retained: %d (%.1f%%)", ncol(data), 100*mean(is_macro)))
